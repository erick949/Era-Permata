...
// This file is the upstream `Container` class essentially unmodified.
// The only changes made for safe integration inside a React/Vite app are
// clearly marked with "REACT-SAFETY PATCH" comments:
//   1) `html2canvas` is imported as an ES module instead of relying on a
//      global <script> tag.
//   2) A `destroy()` method and a static `recapture()` helper were added,
//      because upstream has no teardown path (it leaks a window scroll
//      listener + a WebGL context per instance forever). Without this,
//      React Strict Mode / hot reload / route changes would exhaust the
//      browser's WebGL context limit.
//   3) A `this._destroyed` guard was added around the async callbacks
//      (page-snapshot resolution + image load) so a component that
//      unmounts mid-capture doesn't touch a dead WebGL context.
// No shader code, sizing logic, or visual behavior was changed.
// ---------------------------------------------------------------------------
import html2canvas from 'html2canvas' // REACT-SAFETY PATCH
...
class Container {
    ...
    html2canvas(document.body, {
      ...
      .then(snapshot => {
        ...
        waitingContainers.forEach(container => {
          if (!container._destroyed && !container.webglInitialized) {
            // REACT-SAFETY PATCH: skip instances unmounted mid-capture
            container.initWebGL()
          }
        })
      })
  ...
  initWebGL() {
    ...
    img.onload = () => {
      if (this._destroyed || !this.gl) return // REACT-SAFETY PATCH
  ...
  startRenderLoop() {
    ...
    const handleScroll = () => {
      if (this._destroyed) return // REACT-SAFETY PATCH
      render()
    }
    window.addEventListener('scroll', handleScroll, { passive: true })
    this._handleScroll = handleScroll // REACT-SAFETY PATCH: keep ref so destroy() can remove it
  ...
  // REACT-SAFETY PATCH ------------------------------------------------------
  // Upstream has no teardown method. This tears down everything a mounted
  // instance holds onto: the static registry entry, the pending snapshot
  // queue entry, the window scroll listener, the WebGL context, and the DOM
  ...
  destroy() {
    ...
    const qIdx = Container.waitingForSnapshot.indexOf(this)
    if (qIdx > -1) Container.waitingForSnapshot.splice(qIdx, 1)
  ...
  // REACT-SAFETY PATCH: force every live instance to re-sample the page.
  // Upstream only ever captures once (the first time a Container inits) and
  // has no way to refresh it, so the glass would otherwise show a stale
  // frame after a layout-changing resize.
  static recapture() {
    Container.pageSnapshot = null
    if (Container.isCapturing) return

    const active = Container.instances.filter(instance => !instance._destroyed)
    ...
    Container.isCapturing = true
    Container.waitingForSnapshot = active.slice()
    active[0].capturePageSnapshot()
  }
  // ---------------------------------------------------------------------
...
export { Container } // REACT-SAFETY PATCH: expose as ES module export
